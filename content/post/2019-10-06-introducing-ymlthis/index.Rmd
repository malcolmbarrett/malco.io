...
`yml()` is the basic building-block of ymlthis; by default, it adds your name and R code to write the date programatically. 
...
ymlthis prints cleanly to the console, so you can easily copy and paste it into your R Markdown document (or use `use_yml()` to programatically place it on your clipboard). However, this might not fit the needs of your workflow. `use_rmarkdown()` and friends take `yml` objects and write them to R Markdown files.
